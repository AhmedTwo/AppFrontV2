...
const submitForm = async () => {
  ...
  let autoEmail = '' // Email de connexion généré automatiquement pour l'administrateur
  ...
  try {
    ...
    const companyFormData = new FormData() // Crée un objet FormData pour l'envoi de données multi-parties (nécessaire pour les fichiers/uploads)
    ...
    const companyResponse = await axios.post(
      ...
      'http://127.0.0.1:8000/api/addCompany', // URL de l'API Laravel
    ...
    console.log("Étape 2 : Envoi des données de l'utilisateur (avec Company ID)...") // Génération des identifiants (Email et Mot de passe brut)
    ...
    autoPassword = generateRandomPassword(8) // Génère un mot de passe aléatoire de 8 caractères

    const userFormData = new FormData() // Crée un nouvel objet FormData pour les données utilisateur
    ...
    userFormData.append('company_id', newCompanyId) // **Liaison essentielle :** utilise l'ID obtenu à l'étape 1
    ...
    userFormData.append('password', autoPassword) // Envoie le mot de passe brut (l'API devrait le hacher)
    userFormData.append('disponibilite', 0) // Définit la disponibilité par défaut à 0 (ou selon la logique métier)
    ...
    console.log("Étape 3 : Déclenchement de l'envoi des identifiants par e-mail...") // Prépare l'objet de données nécessaire à l'API d'envoi d'e-mail
    ...
    const emailPayload = {
      ...
      user_login_email: autoEmail, // Login généré (pour le contenu de l'e-mail)
      user_raw_password: autoPassword, // Mot de passe généré (pour le contenu de l'e-mail)
      user_full_name: `${userData.value.prenom} ${userData.value.nom}`, // Nom complet de l'administrateur

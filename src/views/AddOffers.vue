...
const addOffer = async () => {
  ...
  const payload = {
    ...
    mission: offerData.value.mission,
    location: offerData.value.location,
    category: offerData.value.category,
    employment_type_id: offerData.value.contract_id,
    technologies_used: offerData.value.technologies_used,
    benefits: offerData.value.benefits,
  }

  try {
    const response = await axios.post('http://127.0.0.1:8000/api/addOffer', payload, {
